...
//		System.out.println("Width B: " + getWidth() + " | " + getHeight());
	...
	public void drawElement(Graphics g, int x, int y, int menuWidth, int menuHeight) {
		...
		if(!sizeCalculated || getWidth() < menuWidth)
			calcSize(menuWidth, menuHeight, stringWidth, (int)textHeight);
...
//		g.setColor(Color.RED); //bounds debug
//		g.drawRect(boxBounds.x, boxBounds.y, boxBounds.width, boxBounds.height);
	...
	public void calcBounds(int x, int y, int menuWidth, int menuHeight) {
		boxBounds.setBounds(x + marginSizeHori, y + ( (getHeight() - boxSize) / 2 ), boxSize, boxSize);
	}
}
